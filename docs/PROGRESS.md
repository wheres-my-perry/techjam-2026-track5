...
**Iteration 1 (job 34, resnet_ft random-size crop, content-fixed data): GENERAL 0.787** — ddpm holdout 0.832/0.800/0.721, official 0.829/0.774/0.647. Worst cell resize_0.25x = inputs below CROP_MIN; vote wrapper now upscales tiny inputs to CROP_MIN (label-independent, the one sanctioned upscale); re-eval job 35. Shortcut hunt: the >=0.99 rows are face_synthetics/sfhq/star_gan (+cips) — dumb-separable (colour, file size), excluded from claims.
